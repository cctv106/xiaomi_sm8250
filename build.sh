...
EOF
    ...
    # 保留原始Makefile内容（除了版本行）
    grep -v "KSU_VERSION_API" Makefile >> Makefile.new
    grep -v "KSU_VERSION_FULL" Makefile.new > Makefile.final
    cat Makefile.final > Makefile
    rm Makefile.{new,final}
    
    cd ..
    
    # 6. 简化KSUVER计算
    KSU_VERSION=23700  # 固定值避免git依赖
    
    # 7. 直接输出版本信息
    echo "=== SukiSU-Ultra 版本信息 ==="
